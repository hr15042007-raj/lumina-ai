            ...
            with st.spinner("Analyzing Knowledge Graph & Scaling Math..."):
                ...
                try:
                    ...
                    if "[QUIZ]" in full_response and include_extras == "YES":
                        ...
                        # CLEANING LOGIC: Removes "[QUIZ] NO" or tags from the misconceptions box in Long mode
                        raw_misc = full_response.split("[MISCONCEPTIONS]")[1].strip()
                        misconceptions = raw_misc.replace("[QUIZ] NO", "").replace("[QUIZ]", "").split("[PRACTICE]")[0].strip()
